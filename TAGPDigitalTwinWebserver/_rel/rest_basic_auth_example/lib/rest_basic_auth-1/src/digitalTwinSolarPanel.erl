...
init(Number) ->
	...
	solarPanelStatus:setPidLink(Number,Pid),
	Pid.
...
%-------------------------------------------
	...
	solarPanelStatus:setStatus({led ,Number,status},off),
	solarPanelStatus:setStatus({rwLed ,Number,status},0),
...
%-------------------------------------------
	...
	solarPanelStatus:setStatus({Number,resResistor},330),
...
% Digital twin of powerSource 
	...
	solarPanelStatus:setStatus({Number,sourceVoltige},3.3),
	%register(source, PI5),
...
%--------------------------------------------
	...
	solarPanelStatus:setStatus({Number,totalCurrent},0),
	solarPanelStatus:setStatus({Number,rwTotalCurrent},0),
	solarPanelStatus:setStatus({Number,totalCurrent},0),
%---------------------------------------------
	...
	solarPanelStatus:setStatus({Number,reswire1},0),
...
loop(AggrInst_Pid, FluidumInst_Pid,PidLed,PIampMeter,PIsource,PIw1,PIW2,PIW3,Number,PIRes) -> 
	receive 
		...
		{led,on} -> 
			...
			%[ {ontvanger,'pi@192.168.0.108'} ! {med, numer, on}],
			solarPanelStatus:updateStatus({led ,Number,status},on),
			loop(AggrInst_Pid, FluidumInst_Pid,PidLed,PIampMeter,PIsource,PIw1,PIW2,PIW3,Number,PIRes);
		{ledResistance, Value} -> 
		...
		{led, isOn} -> 
			...
			solarPanelStatus:updateStatus(Respons),
			digitalTwinStatus:setStatus([led, status],Respons),
			solarPanelStatus:updateStatus({rwLed ,Number,status},on),
			%[ {ontvanger,'pi@192.168.0.108'} ! {led, numer, isOn}],
			loop(AggrInst_Pid, FluidumInst_Pid,PidLed,PIampMeter,PIsource,PIw1,PIW2,PIW3,Number,PIRes);
		{source, getVoltige} -> 
