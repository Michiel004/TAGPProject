...
init() -> 
	survivor:entry(powerDissipationTyp_created),
	loop().
	...
	receive
		{initial_state, {ResInst_Pid, {WireInst_Pid, RealWorldCmdFn}}, ReplyFn} ->
			ReplyFn(#{resInst => ResInst_Pid, wireInst => WireInst_Pid, 
					  rw_cmd => RealWorldCmdFn, on_or_off => off,resistance_Vaulue => 0,voltige => 0}), 
			loop();
		...
		{{setResistance, Value}, State, ReplyFn} -> 
			...
			loop(); 
		{{setVoltige, Value}, State, ReplyFn} -> 
			#{rw_cmd := ExecFn} = State, ExecFn(Value), 
			ReplyFn(State#{voltige := Value}),
			loop(); 
		{isResistance, State, ReplyFn} -> 
			#{resistance_Vaulue := ResistanceVaulue} = State, 
			ReplyFn(ResistanceVaulue),
			loop();
		{getVoltige, State, ReplyFn} -> 
			#{resistance_Vaulue := Voltige} = State, 
			ReplyFn(Voltige),
			loop();
