-module(powerSourceInst).
-export([create/4, init/4, switch_on/1, switch_off/1, is_on/1, flow_influence/1, subscribe/2,is_resistance/1,set_resistance/2,get_voltige/1,set_voltige/2]).
...
set_voltige(PowerDissipationTyp_Pid,Value) ->
	...
	survivorUser:entry({PowerDissipationTyp_Pid,voltigeistanceIsSet,Value}).

is_resistance(PowerDissipationTyp_Pid) -> 
	msg:get(PowerDissipationTyp_Pid, isResistance). 

get_voltige(PowerDissipationTyp_Pid) -> 
	msg:get(PowerDissipationTyp_Pid, isVoltige).
...
loop(Host, State, PowerDissipationTyp_Pid, WireInst_Pid, Subcriber_List) -> 
	receive
		...
		{setResistance, Value} -> 
			...
			loop(Host, NewState, PowerDissipationTyp_Pid, WireInst_Pid, Subcriber_List);
		{setVoltige, Value} -> 
			{ok, NewState} = msg:set_ack(PowerDissipationTyp_Pid, {setVoltige, Value}, State),
			notify_subscribers(Subcriber_List, {self(), setVoltige}), 
			loop(Host, NewState, PowerDissipationTyp_Pid, WireInst_Pid, Subcriber_List);
		{isResistance, ReplyFn} -> 
			...
			loop(Host, State, PowerDissipationTyp_Pid, WireInst_Pid, Subcriber_List);
		{isVoltige, ReplyFn} -> 
			{ok, Answer} = msg:get(PowerDissipationTyp_Pid, isVoltige, State), 
			ReplyFn(Answer), 
			loop(Host, State, PowerDissipationTyp_Pid, WireInst_Pid, Subcriber_List);
		{get_type, ReplyFn} -> 
